...
export default function ClassNode({ data, selected }) {
  ...
  const displayedProperties = data.kind === 'enum' ? (data.literals || []).map((literal) => ({ visibility: '', name: literal, type: '' })) : data.kind === 'interface' ? [] : data.properties;
  return <div onContextMenu={data.canManage ? openMenu : undefined} className="group relative min-w-[250px] overflow-visible font-mono text-xs">
    ...
    <div className="space-y-1 border-b border-slate-700/70 px-3 py-2.5 text-slate-200">{displayedProperties?.length ? displayedProperties.map((prop, index) => <div className="flex justify-between gap-5" key={`${prop.name}-${index}`}><span><b className="mr-1 text-cyan-300">{prop.visibility}</b>{prop.name}{prop.embedded && <small className="ml-1 text-cyan-300">@Embedded</small>}</span><span className="text-slate-400">{prop.type}</span></div>) : <span className="text-slate-500">{data.kind === 'enum' ? 'Sin literales' : 'Sin atributos'}</span>}</div>
