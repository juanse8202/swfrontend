...
export default function RelationEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, markerEnd }) {
  ...
  const [panelPosition, setPanelPosition] = useState(() => ({ x: Math.max(16, window.innerWidth - 420), y: 96 }));
  ...
  return <>
    ...
    <EdgeLabelRenderer>
      ...
      <div style={{ position: 'absolute', transform: `translate(-50%, -50%) translate(${labelX}px, ${labelY}px)`, pointerEvents: 'all' }} className="nodrag nopan">
        ...
        {open && createPortal(<><div onClick={() => setOpen(false)} className="fixed inset-0 z-[9998] bg-[#020617]/85" /><section style={{ backgroundColor: '#0b1430', opacity: 1, left: panelPosition.x, top: panelPosition.y }} className="fixed z-[9999] w-[390px] overflow-hidden rounded-2xl border-2 border-indigo-400/60 p-4 text-left text-xs text-slate-200 shadow-[0_0_35px_rgba(99,102,241,0.35)]">
